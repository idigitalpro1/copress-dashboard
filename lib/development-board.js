...
const columns = new Set(['backlog', 'active', 'review', 'blocked', 'done']);
export function normalizeCards(rows) {
  ...
  return rows.filter(row => {
    if (!row || !known.has(row.id) || seen.has(row.id)) return false;
    seen.add(row.id);
    return true;
  }).map(row => {
    ...
    return { ...original, title: String(row.title || '').replace(/^\[[^\]]+\]\s*/, '').slice(0,180) || original.title,
      ...
      blocked: row.column === 'blocked' || (row.column === 'backlog' && original.blocked),
...
export async function getBoard(fetcher = fetch) {
  ...
  try {
    ...
    return {board:snapshot.board, sourceMode:'live', checkedAt, snapshotUpdatedAt:snapshot.updatedAt, source:snapshot.source, consolidation:snapshot.consolidation, cards};
  } catch {
    return {board:snapshot.board, sourceMode:'snapshot', checkedAt, snapshotUpdatedAt:snapshot.updatedAt, source:snapshot.source,
      warning:'Live board could not be confirmed. Showing the dated release snapshot; edits may not be reflected.', consolidation:snapshot.consolidation, cards:snapshot.cards};
  }
}
