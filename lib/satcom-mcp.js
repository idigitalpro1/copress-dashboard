...
import { z } from 'zod';
import { getBoard } from './development-board.js';
...
export const promptNames = ['astra-development', 'claude-development', 'grok-development', 'hermes-development', 'satcom-release', 'partners-editorial'];
...
export function buildServer() {
  const server = new McpServer({ name: 'satcom-operations', version: '1.3.0' });
  const annotations = { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false };
  const result = value => ({ content: [{ type: 'text', text: JSON.stringify(value) }], structuredContent: value });
  server.registerTool('satcom_board', {
    title: 'SATCOM development Kanban', description: 'Read reviewed development cards from the existing shared Kanban, with explicit live or snapshot status. Use a small limit to conserve context.',
    inputSchema: { project: z.enum(['Network','RegisterCall','Villager','SATCOM','Admin','Subscriptions','Ace','Cursor','Legacy sprint']).optional(), key: z.string().regex(/^(DEV|PROV)-\d{3}$/).optional(), offset: z.number().int().min(0).max(10000).default(0), limit: z.number().int().min(1).max(50).default(10) },
    annotations: { ...annotations, openWorldHint: true }
  }, async ({ project, key, offset, limit }) => {
    const board = await getBoard();
    const selected = board.cards.filter(card => (!project || card.project === project) && (!key || card.key === key));
    const { cards, ...metadata } = board;
    return result({ ...metadata, total: selected.length, offset, nextOffset: offset + limit < selected.length ? offset + limit : null, cards: selected.slice(offset,offset+limit) });
  });
  ...
  }, async () => result({ updated: context.updated, site: context.site, architecture: context.architecture, operatorPolicy: context.operatorPolicy, routes: context.routes, boundaries: context.boundaries, combinedBoard: context.combinedBoard, latestRelease: context.latestRelease }));
