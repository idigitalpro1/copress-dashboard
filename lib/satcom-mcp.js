...
export function buildServer() {
  ...
  server.registerTool('satcom_board', {
    ...
    inputSchema: { project: z.enum(['Network','RegisterCall','Villager','SATCOM','Ace','Cursor','Legacy sprint']).optional(), limit: z.number().int().min(1).max(50).default(10) },
    annotations: { ...annotations, openWorldHint: true }
  }, async ({ project, limit }) => {
    const board = await getBoard();
    const selected = board.cards.filter(card => !project || card.project === project);
    const { cards, ...metadata } = board;
    return result({ ...metadata, total: selected.length, cards: selected.slice(0,limit) });
  });
  ...
  }, async () => result({ updated: context.updated, site: context.site, architecture: context.architecture, operatorPolicy: context.operatorPolicy, routes: context.routes, boundaries: context.boundaries }));
