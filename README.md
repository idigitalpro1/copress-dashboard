...
- Local stories expose a guarded **Publish Draft** handoff only after reaching `Ready`. The action stores `localStorage.copress_editorial_publish_handoff_v1`, copies a structured publish package when clipboard access is available, and opens the canonical Villager Admin scoped to Register Call for the authenticated final publish step. SATCOM does not perform browser-side writes with a Supabase service-role key.
